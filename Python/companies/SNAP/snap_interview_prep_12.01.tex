...
\textbf{Situation:} "At DataHub, we needed to migrate 500M+ records from a monolithic search index to a multi-tenant architecture serving 10K+ organizations. The existing system had 2s p99 latency and frequent OOM crashes."
...
\textbf{How to Present This:}
\begin{itemize}
    \item "At DataHub, we built hybrid search combining BM25 and vector embeddings. For ads, I'd use the same approach - rule-based targeting for precision (exact demographic matches) and semantic embeddings for recall (discover similar interests)."
...
\textbf{How to Present This:}
\begin{itemize}
    \item "We used sentence transformers for semantic search at DataHub. For Snapchat ads, I'd encode user behavior (Stories watched, Lenses used) into embeddings and match against ad embeddings for semantic targeting."
...
\textbf{How to Present This:}
\begin{itemize}
    \item "At DataHub, we built multi-tenant search with strict workspace isolation. Ads systems have similar requirements - each advertiser's budget must be tracked separately, campaigns must respect spend limits, just like workspace quotas."
...
"Great question! Before diving in, I want to clarify - in my experience building search systems at DataHub, I've found that ad serving is fundamentally a retrieval and ranking problem. The core challenge is:
...
"I want to start by noting that in my experience building large-scale search systems at DataHub, I've found that \textbf{ad serving is fundamentally a retrieval and ranking problem}.
